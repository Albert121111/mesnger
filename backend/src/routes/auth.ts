...
router.post('/register', async (req, res) => {
  ...
  const user = await prisma.user.create({ data: { email: body.email.toLowerCase(), passwordHash: await hashPassword(body.password), username: body.username.toLowerCase().replace('@', ''), displayName: body.displayName, settings: { create: {} } }, include: { settings: true } });
